...
void main() {
  ...
  testWidgets('ux commands dispatch generator and full callbacks',
      (tester) async {
    final controller = TextEditingController();
    final executed = <String>[];
    await tester.pumpWidget(_harness(
    ...
    await tester.pump();
    await tester.tap(find.text('/ux  UX'));
    await tester.pump();
  ...
  testWidgets('ux is blocked without a project workspace', (tester) async {
    final controller = TextEditingController();
    final executed = <String>[];
    await tester.pumpWidget(_harness(
      controller: controller,
    ...
    expect(
      find.text('Choose a project chat with a workspace before starting UX.'),
      findsWidgets,
    );
    expect(executed, isEmpty);
  });
