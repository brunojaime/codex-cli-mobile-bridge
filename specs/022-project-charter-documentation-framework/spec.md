...
- natural-language requests can update the charter or a specific charter
  module;
- UX Generator, functional Generator, Reviewer, and final UX passes read the
  current Acta and synchronize confirmed project facts without implicitly
  delivering it;
- the mobile reader uses comfortable phone typography and can email one or
  more recipients, including the complete document by default;
...
- action to share by email with multiple recipients, an optional message, and
  complete-document inclusion enabled by default;
...
- `POST /sdd/project/charter/share`
...
- AC-014: The Project Factory agent sequence reads and evolves the working Acta
  while preserving `draft` state until explicit client delivery.
- AC-015: The mobile document surface is readable at phone size and shares the
  complete Acta with multiple email recipients by default.
