...
- [x] T059 Gate full Project Factory execution on a valid formal v0.1 Acta instead of a synthetic approved v1.0 file.
- [x] T060 Require UX Generator, functional Generator, Reviewer, and final UX prompts to read and synchronize the working Acta without implicit delivery.
- [x] T061 Unify the SDD compatibility reader and email endpoint with `acta/current/acta.md` and formal YAML metadata.
- [x] T062 Attach the complete Markdown and rendered HTML when SMTP sharing includes the full document.
- [x] T063 Add a mobile multi-recipient share dialog with complete-document inclusion enabled by default and comfortable phone typography.
- [x] T064 Test formal draft reading, integrity checks, full email attachments, narrow-phone sharing, and delivered snapshot metadata.
