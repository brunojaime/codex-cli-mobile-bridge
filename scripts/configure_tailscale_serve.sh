...
CONTROL_TARGET_URL="http://127.0.0.1:8010"
...
configure_serve() {
  ...
  if ! proxy_matches "${status}" "443" "/" "${TARGET_URL}"; then
    tailscale "${tailscale_args[@]}" serve --bg --https=443 --yes "${TARGET_URL}"
  fi

  status="$(tailscale "${tailscale_args[@]}" serve status --json 2>/dev/null || printf '{}')"
  if ! proxy_matches "${status}" "80" "/" "${TARGET_URL}"; then
    tailscale "${tailscale_args[@]}" serve --bg --http=80 --yes "${TARGET_URL}"
  fi

  if [[ ! -f "${ROOT_DIR}/.control.env" ]]; then
    return
  fi

  status="$(tailscale "${tailscale_args[@]}" serve status --json 2>/dev/null || printf '{}')"
  if ! proxy_matches "${status}" "443" "/ops" "${CONTROL_TARGET_URL}"; then
    tailscale "${tailscale_args[@]}" serve --bg --https=443 --set-path=/ops --yes "${CONTROL_TARGET_URL}"
  fi

  status="$(tailscale "${tailscale_args[@]}" serve status --json 2>/dev/null || printf '{}')"
  if ! proxy_matches "${status}" "80" "/ops" "${CONTROL_TARGET_URL}"; then
    tailscale "${tailscale_args[@]}" serve --bg --http=80 --set-path=/ops --yes "${CONTROL_TARGET_URL}"
  fi
}
...
proxy_matches() {
  ...
  local path="$3"
  local target="$4"
  STATUS_JSON="${status_json}" python3 - "${port}" "${path}" "${target}" <<'PY'
import json
import os
import sys
