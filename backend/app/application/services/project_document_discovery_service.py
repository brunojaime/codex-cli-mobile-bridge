...
class ProjectDocumentDiscoveryService:
    ...
    def _workspace_from_generation_job(self, job_id: str) -> Path | None:
        ...
        manifest_plan = getattr(job, "manifest_plan", None) if job is not None else None
        target_path = getattr(manifest_plan, "target_path", None)
        candidate = Path(project_path or target_path) if project_path or target_path else None
    ...
    def _workspace_from_generation_draft(self, draft_id: str) -> Path | None:
        ...
        for job in self._project_factory_service.list_jobs(draft_id=draft_id, limit=20):
            if not isinstance(job, dict):
                continue
            candidate_value = job.get("project_path") or job.get("target_path")
            if candidate_value and Path(str(candidate_value)).is_dir():
                return Path(str(candidate_value))
        return None
