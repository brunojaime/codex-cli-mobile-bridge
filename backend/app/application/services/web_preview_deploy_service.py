...
class CloudflarePreviewProvisioner:
    ...
    def apply(self, *, manifest: dict[str, Any], project_path: Path) -> list[dict[str, Any]]:
        ...
        r2_bucket_name = str(r2_bucket or "")
        r2_binding = str(resources.get("r2_binding") or "DOCUMENTS_BUCKET") if r2_bucket_name else ""
    ...
    *,
    ...
    if r2_bucket_name and r2_binding:
        lines.extend(
            [
                "[[r2_buckets]]",
                f'binding = "{_toml_escape(r2_binding)}"',
                f'bucket_name = "{_toml_escape(r2_bucket_name)}"',
                "",
            ]
        )
    lines.extend(
