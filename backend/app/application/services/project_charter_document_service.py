...
class ProjectCharterDocumentService:
    ...
    def release(
        ...
        *,
        ...
        self._write_yaml(metadata_path, metadata)
        self.refresh_render()
        regenerated_pdf = self.export_pdf() if client_export else None
        if regenerated_pdf is not None and not regenerated_pdf.ok:
            return ProjectCharterReleaseResult(
                ok=False,
                version=version,
                release_path=None,
                validation=regenerated_pdf.validation,
                recommended_impact=recommended,
            )
