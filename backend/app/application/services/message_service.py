        ...
        *,
        ...
        if document_kind == "video":
            video_reference = self._build_video_file_reference(
                video_path=str(stored_document_path),
                content_type=content_type,
            )
            prompt = self._build_document_execution_message(
                message=message,
        ...
        if document_kind == "zip":
            extracted_text = self._append_zip_archive_guidance(
                extracted_text,
                archive_path=str(stored_document_path),
            )
        ...
        if document_kind in {"docx", "text", "pdf", "pptx", "xlsx", "zip"}:
            prompt = self._build_document_execution_message(
                message=message,
                document_kind=document_kind,
                document_name=attached_document_name,
        ...
        *,
        ...
        if suffix == ".zip":
            if (
                ...
                and normalized_content_type not in _ZIP_UPLOAD_CONTENT_TYPES
            ):
                raise UnsupportedDocumentError(
                    "Unsupported MIME type for .zip uploads: "
        ...
        if normalized_content_type.startswith("audio/"):
            return "audio"
        if normalized_content_type in _ZIP_UPLOAD_CONTENT_TYPES:
            return "zip"
