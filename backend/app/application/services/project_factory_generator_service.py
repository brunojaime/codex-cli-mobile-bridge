...
def _mobile_android_manifest(name: str) -> str:
    app_label = _android_label(name)
...
def _android_label(name: str) -> str:
    label = re.sub(r"\s+", " ", name.replace("-", " ")).strip()
    if not label:
        label = "Preview"
    return (
        label.replace("&", "&amp;")
